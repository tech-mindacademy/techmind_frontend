...
import {
  ...
  Monitor, BarChart2, Brain, Shield, Cloud, Smartphone,
  Paintbrush, Link2, Gamepad2, FlaskConical, Briefcase,
  Zap, CheckCircle, AlertCircle, SlidersHorizontal, Lock, ArrowRight,
} from "lucide-react";
...
// ── Payment step (shown inside the modal when the internship has a fee) ────
function PaymentStep({ applicationId, amount, internshipTitle, onDone }) {
  ...
  const startPayment = async () => {
    ...
    try {
      ...
      const order = await createInternshipCheckout(applicationId);
      ...
      const options = {
        ...
        name: "Tech Vidya",
        description: `Internship Fee — ${internshipTitle}`,
        ...
        handler: async (response) => {
          try {
            await verifyInternshipPayment(applicationId, {
            ...
            setErr(e.response?.data?.message || "Payment verification failed. Contact support.");
          } finally {
            setPaying(false);
          }
        },
      ...
      setErr(e.message || "Something went wrong starting payment.");
      setPaying(false);
    }
  };
  ...
  return (
    <div className="p-10 text-center">
      <div className="w-20 h-20 rounded-full bg-amber-50 border-2 border-amber-200 flex items-center justify-center mx-auto mb-5">
        <Lock className="w-9 h-9 text-amber-500" strokeWidth={1.5} />
      </div>
      <h3 className="text-2xl font-black text-[#1A56DB] mb-2">One Last Step — Payment</h3>
      <p className="text-black/50 mb-6 max-w-sm mx-auto text-sm leading-relaxed">
        Complete the internship fee to unlock your <strong>Internship Portal</strong> and live class access.
      </p>
      {err && <p className="text-red-500 text-sm mb-4">{err}</p>}
      <button
        onClick={startPayment}
        disabled={paying}
        className="bg-[#1A56DB] hover:bg-[#0D1B3E] disabled:opacity-60 text-white font-black px-8 py-3 rounded-xl transition inline-flex items-center gap-2"
      >
        {paying ? (
          <>
...
export default function InternshipsPage() {
  ...
  const [internships, setInternships]   = useState([]);
  const [access, setAccess]             = useState([]); // my-access list, drives Apply→Join button
  const [loading, setLoading]           = useState(true);
  const [search, setSearch]             = useState("");
  const [typeFilter, setTypeFilter]     = useState("");
  const [domain, setDomain]             = useState("All");
  ...
  // Fetch "my access" so Apply buttons can flip to Join Portal for internships
  // the logged-in user is already shortlisted (and paid, if required) for.
  useEffect(() => {
    if (!isAuthenticated) {
      setAccess([]);
      return;
  ...
  const getAccessFor = (intern) =>
    access.find(
      (a) =>
        a.internshipId === intern._id ||
        (a.internshipTitle === intern.title && a.company === intern.company)
    );
  ...
  const openApply = (intern) => {
    // Paid internships require an account so payment + portal access can be
    // linked to it — send anonymous visitors to log in first rather than
    // letting them fill out a form they can never pay for.
    if ((intern.applicationFee > 0) && !isAuthenticated) {
      navigate("/auth", { state: { from: { pathname: "/internships" } } });
      return;
    }
    setApplying(intern);
    setForm(BLANK_FORM);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let data;
      if (applying._id.startsWith("s")) {
        const res = await api.post("/internships/apply-static", {
          internship: {
            title: applying.title, company: applying.company, domain: applying.domain,
            location: applying.location, duration: applying.duration, stipend: applying.stipend,
            type: applying.type, openings: applying.openings, skills: applying.skills,
        ...
        const res = await api.post(`/internships/${applying._id}/apply`, form);
        data = res.data;
      }
      setApplicationId(data.applicationId);
      setPaymentRequired(!!data.requiresPayment);
      setSuccess(true);
    } catch (err) {
      setError(err.response?.data?.message || "Failed to submit. Please try again.");
    } finally {
                      ...
                      <div className="mt-auto pt-3 border-t border-[#0D1B3E]/8 flex items-center justify-between gap-2">
                        ...
                        {acc?.canJoinPortal ? (
                          <button
                            ...
                            className="px-4 py-2 rounded-xl text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white shadow transition flex items-center gap-1.5"
                          >
                            Join Portal <ArrowRight className="w-3.5 h-3.5" />
                          </button>
                        ) : acc?.status === "shortlisted" && acc?.requiresPayment && acc?.paymentStatus !== "paid" ? (
                          <button
                            onClick={() => openApply(intern)}
                            className="px-4 py-2 rounded-xl text-xs font-bold bg-amber-500 hover:bg-amber-600 text-white shadow transition flex items-center gap-1.5"
                          >
                            <Lock className="w-3.5 h-3.5" /> Complete Payment
                          </button>
                        ) : (
                          <button
                            onClick={() => !expired && openApply(intern)}
        ...
        {applying && (
          ...
          >
            ...
            >
              ...
              {/* ── Success / payment / form states ── */}
              {success && paymentRequired && !paymentDone ? (
                <PaymentStep
                  applicationId={applicationId}
                  amount={applying.applicationFee}
                  internshipTitle={applying.title}
                ...
                <div className="p-12 text-center">
                  ...
                  <div className="flex items-center justify-center gap-3">
                    {isAuthenticated && (
                      <button
                        onClick={() => { setApplying(null); navigate("/intern/dashboard"); }}
                        className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-6 py-3 rounded-xl transition flex items-center gap-2"
                      >
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                  ...
                  <div className="bg-[#1A56DB]/5 border border-[#1A56DB]/15 rounded-xl px-4 py-3 flex items-start gap-2">
                    <Zap className="w-4 h-4 text-[#1A56DB] flex-shrink-0 mt-0.5" strokeWidth={2} />
                    <p className="text-xs text-[#1A56DB] leading-relaxed">
                      Applications are <strong>automatically shortlisted</strong>.
                      {applying.applicationFee > 0
                        ? ` A one-time fee of ₹${applying.applicationFee} unlocks your Internship Portal and live classes.`
                        : " Fill out all required fields accurately — our team reviews every submission personally."}
                    </p>
                  </div>
