...
import { markLessonComplete, updateLastAccessed, fetchEnrollment } from "../../api/services/course.service";
...
const SidebarTab = ({ active, onClick, children }) => (
  <button onClick={onClick}
    className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-all ${active ? "bg-indigo-600 text-white" : "text-gray-400 hover:text-white hover:bg-gray-800"}`}>
    {children}
  </button>
);
...
export default function CoursePlayerPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setIsLoading(true);
        // Use axios (has auth interceptor built-in)
        const courseRes = await api.get(`/courses/${courseId}`).then(r => r.data);
        if (!courseRes.success) { toast.error("Course not found"); return; }
        ...
        if (enrollRes?.enrollment) {
          const done = new Set(
            enrollRes.enrollment.completedLessons?.map(cl => cl.lesson?.toString()) || []
          );
  ...
  const selectLessonFromCourse = (lessonId, courseData) => {
    for (const sec of courseData.sections || []) {
      const lesson = sec.lessons.find(l => l._id === lessonId || l._id?.toString() === lessonId?.toString());
      if (lesson) {
  ...
  const selectLesson = (lessonId) => {
    ...
    navigate(`/student/learn/${courseId}/lesson/${lessonId}`, { replace: true });
  ...
  const currentIdx = allLessons.findIndex(l => l._id === activeLessonId || l._id?.toString() === activeLessonId?.toString());
  const progress = allLessons.length > 0
    ? Math.round((completedLessons.size / allLessons.length) * 100)
    : 0;

  const isDone = (id) => completedLessons.has(id) || completedLessons.has(id?.toString());
    ...
    <div className="h-screen flex flex-col bg-gray-950 overflow-hidden">
      ...
      <div className="h-14 bg-gray-900 border-b border-gray-800 flex items-center px-4 gap-3 flex-shrink-0 z-10">
        ...
        <div className="flex-1 min-w-0">
          ...
          {activeLesson && (
            <p className="text-xs text-gray-500 truncate">{activeSection?.title} · {activeLesson.title}</p>
          )}
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          {/* Video */}
          <div className="bg-black flex-shrink-0 w-full" style={{ aspectRatio: "16/9", maxHeight: "62vh" }}>
            {activeLesson?.video?.url ? (
              ...
              <div className="w-full h-full flex flex-col items-center justify-center gap-3 text-gray-700">
                ...
                <p className="text-sm">{activeLesson ? "No video for this lesson" : "Select a lesson to begin"}</p>
              </div>
            )}
          </div>
            ...
            {activeLesson ? (
              <div className="max-w-2xl">
                <div className="flex items-start justify-between gap-4 mb-4 flex-wrap">
                  ...
                  <button onClick={handleMarkComplete} disabled={isDone(activeLessonId)}
                ...
                {activeLesson.notes?.length > 0 && (
                  <div className="mb-5">
                    <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Lesson Materials</p>
                    <div className="flex flex-wrap gap-2">
                      {activeLesson.notes.map(note => (
                        <a key={note._id} href={note.url} target="_blank" rel="noreferrer"
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-800">
                  <button
                    onClick={() => { const prev = allLessons[currentIdx - 1]; if (prev) selectLesson(prev._id); }}
                  ...
                  <span className="text-xs text-gray-600">{currentIdx + 1} / {allLessons.length}</span>
                  <button
                    onClick={() => { const next = allLessons[currentIdx + 1]; if (next) selectLesson(next._id); }}
        ...
        <AnimatePresence initial={false}>
          {sidebarOpen && (
            <motion.div initial={{ width: 0 }} animate={{ width: 340 }} exit={{ width: 0 }}
              transition={{ type: "spring", damping: 30, stiffness: 280 }}
              className="flex-shrink-0 border-l border-gray-800 bg-gray-900 flex flex-col overflow-hidden"
              style={{ minWidth: 0 }}>
              {/* Tab bar */}
              <div className="p-2.5 border-b border-gray-800 flex gap-1 flex-wrap">
                <SidebarTab active={sidebarTab === "content"} onClick={() => setSidebarTab("content")}>
                  Content
                </SidebarTab>
                {activeLesson?.quiz && (
                  <SidebarTab active={sidebarTab === "quiz"} onClick={() => setSidebarTab("quiz")}>
                    Quiz
                  </SidebarTab>
                )}
                {activeLesson?.assignment && (
                  <SidebarTab active={sidebarTab === "assignment"} onClick={() => setSidebarTab("assignment")}>
                    Assignment
                  </SidebarTab>
                )}
              </div>
                  ...
                  {course?.sections?.map(section => (
                    <div key={section._id} className="border-b border-gray-800/60">
                      <button
                        ...
                        <svg className={`w-4 h-4 text-gray-500 transition-transform flex-shrink-0 ${expandedSections[section._id] ? "rotate-180" : ""}`}
                        ...
                        {expandedSections[section._id] && (
                          <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }}
                            ...
                            {section.lessons.map(lesson => {
                              const isActive = lesson._id === activeLessonId || lesson._id?.toString() === activeLessonId?.toString();
                              const done = isDone(lesson._id);
                              return (
                                <button key={lesson._id} onClick={() => selectLesson(lesson._id)}
                                  className={`w-full flex items-start gap-3 px-4 py-3 text-left transition-all ${isActive ? "bg-indigo-900/30 border-r-2 border-indigo-500" : "hover:bg-gray-800/50"}`}>
                                  <div className="mt-0.5 flex-shrink-0">
                                    {done ? (
                                      ...
                                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${isActive ? "border-indigo-400" : "border-gray-600"}`}>
                                        {isActive && <div className="w-2 h-2 rounded-full bg-indigo-400" />}
                                      </div>
                                    )}
                                  </div>
                                  <div className="flex-1 min-w-0">
                                    <p className={`text-sm leading-snug ${isActive ? "text-indigo-300 font-medium" : done ? "text-gray-500" : "text-gray-300"}`}>
                                      {lesson.title}
                                    </p>
                                    <div className="flex flex-wrap items-center gap-1.5 mt-1">
                                      {lesson.video?.duration > 0 && (
                                        <span className="text-xs text-gray-600">{fmtDuration(lesson.video.duration)}</span>
                                      )}
                                      {lesson.quiz && (
                                        <span className="text-xs bg-purple-900/40 text-purple-400 px-1.5 py-0.5 rounded">Quiz</span>
                                      )}
                                      {lesson.assignment && (
                                        <span className="text-xs bg-amber-900/40 text-amber-400 px-1.5 py-0.5 rounded">Assignment</span>
                                      )}
                                      {lesson.isFreePreview && (
                                        <span className="text-xs bg-green-900/40 text-green-400 px-1.5 py-0.5 rounded">Free</span>
                                      )}
                                    </div>
                                  </div>
                                </button>
              ...
              {sidebarTab === "quiz" && activeLesson?.quiz && (
                <div className="flex-1 overflow-y-auto p-4">
                  <QuizPanel quizId={activeLesson.quiz} courseId={courseId} lessonId={activeLessonId} />
                </div>
              )}

              {sidebarTab === "assignment" && activeLesson?.assignment && (
                <div className="flex-1 overflow-y-auto p-4">
                  <AssignmentPanel assignmentId={activeLesson.assignment} courseId={courseId} />
                </div>
              )}
            </motion.div>
          )}
