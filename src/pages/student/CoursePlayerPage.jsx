...
export default function CoursePlayerPage() {
  ...
  const selectLessonFromCourse = useCallback((lessonId, courseData) => {
    for (const sec of courseData.sections || []) {
      ...
      if (lesson) {
        ...
        // Auto-switch tab based on lesson content
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setIsLoading(true);
        const endpoint = isAdmin ? `/courses/preview/${courseId}` : `/courses/${courseId}`;
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-950 overflow-hidden">
      ...
      <div className="h-14 bg-gray-900 border-b border-gray-800 flex items-center px-4 gap-3 flex-shrink-0 z-10">
        <button
          onClick={() => navigate(isAdmin ? "/admin/course-approvals" : "/student/my-courses")}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          {/* ── Content panel ── */}
          <div className="flex-shrink-0 w-full flex flex-col bg-gray-900" style={{ maxHeight: "62vh" }}>
            ...
            {activeLesson && hasExtras && (
              <div className="flex items-center bg-gray-900 border-b border-gray-800 flex-shrink-0 px-1">
                ...
                {activeLesson?.video?.public_id && (
                  <ContentTab active={contentTab === "video"} color="indigo" onClick={() => setContentTab("video")}>
                ...
                {hasQuiz && (
                  <ContentTab active={contentTab === "quiz"} color="purple" onClick={() => setContentTab("quiz")}>
                    <span className="flex items-center gap-1.5">
                      <IconQuiz size={13} /> Quiz
                    </span>
                  </ContentTab>
                )}
                {hasAssignment && (
                  <ContentTab active={contentTab === "assignment"} color="amber" onClick={() => setContentTab("assignment")}>
                    <span className="flex items-center gap-1.5">
                      <IconAssignment size={13} /> Assignment
                    </span>
                  </ContentTab>
                )}
              </div>
            ...
            {contentTab === "quiz" && hasQuiz && (
              <div className="w-full overflow-y-auto bg-gray-950 p-5" style={{ aspectRatio: "16/9", maxHeight: "62vh" }}>
                <QuizPanel
              ...
              <div className="max-w-2xl">
                <div className="flex items-start justify-between gap-4 mb-4 flex-wrap">
                    ...
                    {hasExtras && (
                      <div className="flex gap-2 mt-2">
                        ...
                        {hasAssignment && (
                          ...
                          >
                            <IconAssignment size={12} /> View Assignment
                          </button>
                        )}
                      </div>
                    )}
                  ...
                  {!isAdmin ? (
                    ...
                    >
                      {isDone(activeLessonId) ? <><IconCheck size={14} /> Done</> : "Mark complete"}
                    </button>
                  ) : (
            ...
            <>
              ...
              >
                <div className="p-2.5 border-b border-gray-800 flex gap-1">
                  <SidebarTab active onClick={() => {}}>Course Content</SidebarTab>
                </div>
                      ...
                      >
                        <div>
                          ...
                          <p className="text-xs text-gray-500 mt-0.5">
                            {section.lessons.length} lessons · {section.lessons.filter((l) => isDone(l._id)).length} done
                          </p>
                        </div>
                        <IconChevronDown
                          size={16}
                                ...
                                >
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <p className={`text-sm leading-snug ${
                                      locked ? "text-gray-600" : isActive ? "text-indigo-300 font-medium" : done ? "text-gray-500" : "text-gray-300"
                                    }`}>
                                      {lesson.title}
                                    </p>
                                    <div className="flex flex-wrap items-center gap-1.5 mt-1">
                                      {lesson.video?.public_id && lesson.video?.duration > 0 && (
