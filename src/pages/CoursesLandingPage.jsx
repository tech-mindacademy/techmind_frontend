    ...
    >
      <Link to={`/auth`}>
        ...
        <div className="p-4">
          ...
          <p className="text-xs text-black/45 mt-1 font-medium">
            {course.creator?.name}
          </p>
...
function FilterPanel({
  ...
  const itemClass = (active) =>
    `cursor-pointer text-sm mb-2 transition flex items-center gap-1.5 ${
      active
        ...
        : "text-black/50 hover:text-[#1A56DB]"
    }`;
  ...
  return (
    <div className="p-5">
      ...
      <h3 className="font-black mb-3 text-[#1A56DB] text-sm uppercase tracking-wider">
        Category
      </h3>
      {["All", ...categories].map((c) => (
        <p
          ...
          className={itemClass(category === c)}
        >
          {category === c && (
...
export default function CoursesLandingPage() {
  ...
  const activeFilterCount = [category !== "All", price !== "All"].filter(
    Boolean,
  ).length;
