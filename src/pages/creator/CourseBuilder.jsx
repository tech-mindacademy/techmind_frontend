...
import {
  ...
  togglePublish, reorderSections, reorderLessons,
...
const CATEGORIES = [
  ...
  "Business", "Marketing", "Personal Development", "Photography", "Music", "Other",
];
...
function DragHandle({ handleProps }) {
  return (
    <button
      {...handleProps}
      className="text-gray-600 hover:text-gray-400 cursor-grab active:cursor-grabbing touch-none flex-shrink-0 p-0.5"
    >
...
function Input({ label, error, ...props }) {
  return (
    <div>
      ...
      <input
        {...props}
        className={`w-full px-3 py-2.5 bg-gray-800 border rounded-xl text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:border-teal-500 transition ${
...
// ─── Sortable lesson row ──────────────────────────────────────────────────────
function SortableLessonRow({ lesson, courseId, sectionId, onDelete }) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="flex items-center gap-3 px-4 py-2.5 border-b border-gray-700/50 hover:bg-gray-700/30 transition group"
    >
      ...
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
        ...
        <Link
          to={`/creator/courses/${courseId}/lessons/${lesson._id}`}
...
// ─── Section item (receives drag props from SortableSectionItem) ──────────────
function SectionItem({
  section, courseId, onUpdated, isExpanded, onToggle, isFinal,
  dragRef, dragStyle, dragHandleProps,
}) {
  ...
  const saveTitle = async () => {
    if (isFinal || !title.trim() || title === section.title) {
  ...
  const handleDeleteLesson = async (lessonId, lessonTitle) => {
    ...
    try {
      await deleteLesson(courseId, section._id, lessonId);
      onUpdated();
  ...
  <DragHandle handleProps={dragHandleProps} />
        ...
        <span className="text-xs text-gray-500 flex-shrink-0">{lessons.length} lessons</span>
            ...
            <div className="border-t border-gray-700">
              ...
              {addingLesson ? (
                <div className="flex items-center gap-2 px-4 py-2.5">
                  ...
                  />
                  <button onClick={handleAddLesson} className="text-teal-400 hover:text-teal-300 text-sm font-semibold">
                    Add
                  </button>
                  <button onClick={() => setAddingLesson(false)} className="text-gray-500 hover:text-white text-sm">
                    Cancel
                  </button>
                </div>
              ) : (
...
export default function CourseBuilder() {
  ...
  const [form, setForm] = useState({
    ...
    category: "Web Development", level: "all",
  ...
  useEffect(() => {
    if (course?.sections) setSections(course.sections);
  }, [course]);
  ...
  const handlePublish = async () => {
    if (!course.isPublished) {
      const finalSection = sections.find(s => FINAL_SECTION_PATTERN.test(s.title));
      if (!finalSection) {
        toast.error("A 'Final Quiz' section is required before publishing.");
        return;
      }
      if (!finalSection.lessons?.some(l => l.quiz)) {
        toast.error("The Final Quiz section needs a lesson with a quiz before publishing.");
        return;
      }
    }
    ...
    try {
      ...
      setCourse(prev => ({
        ...
        approvalStatus: res.approvalStatus || prev.approvalStatus,
      }));
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">
              {isEditing ? (course?.title || "Edit Course") : "New Course"}
            </h1>
            {isEditing && (
            ...
            <div className="bg-gray-800 border border-gray-700 rounded-2xl p-5 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  className={`w-full px-3 py-2.5 bg-gray-700 border rounded-xl text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:border-teal-500 resize-none transition ${
          ...
          <div className="space-y-5">
            <div className="bg-gray-800 border border-gray-700 rounded-2xl p-5 space-y-4">
              ...
              <label className="cursor-pointer block">
                <div className={`aspect-video rounded-xl border-2 border-dashed overflow-hidden transition ${
                  thumbnailPreview ? "border-transparent" : "border-gray-600 hover:border-teal-500"
                }`}>
                  {thumbnailPreview ? (
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                {sections.length} sections · {course?.stats?.totalLessons || 0} lessons
                <span className="ml-2 text-gray-600">— drag ⠿ to reorder</span>
              </p>
            </div>
          </div>
              ...
              <div className="space-y-3">
                {sections.map(section => (
                  <SortableSectionItem
                    ...
                    onToggle={() =>
                      setExpandedSections(p => ({ ...p, [section._id]: !p[section._id] }))
                    }
          ...
          {showAddSection ? (
            <div className="flex items-center gap-2 bg-gray-800 border border-teal-500 rounded-xl px-4 py-3">
              ...
              />
              <button onClick={handleAddSection} className="text-teal-400 hover:text-teal-300 text-sm font-semibold">
                Add
              </button>
              <button onClick={() => setShowAddSection(false)} className="text-gray-500 hover:text-white text-sm">
                Cancel
              </button>
            </div>
          ) : (
