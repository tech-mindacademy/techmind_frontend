      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="p-4">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-xs text-indigo-600 dark:text-indigo-400 font-semibold capitalize">{course.level}</span>
          ...
          {course.shortDescription && (
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 line-clamp-2">{course.shortDescription}</p>
          )}
          <div className="flex items-center gap-2 mt-2">
            {course.creator?.avatar?.url ? (
              <img src={course.creator.avatar.url} alt={course.creator.name} className="w-5 h-5 rounded-full" />
            ) : (
          ...
          <div className="flex items-center justify-between mt-3">
            <div className="flex items-center gap-2 text-xs text-gray-500">
              {course.stats?.totalStudents > 0 && <span>👥 {course.stats.totalStudents.toLocaleString()}</span>}
            ...
            <div className="font-bold text-gray-900 dark:text-white text-sm">
              {course.isFree || course.price === 0 ? (
                ...
                <span>₹{(course.discountPrice > 0 ? course.discountPrice : course.price).toLocaleString()}</span>
              )}
            </div>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <div className="bg-gradient-to-br from-indigo-600 to-violet-600 py-12 px-4">
        <div className="max-w-4xl mx-auto text-center space-y-5">
          <h1 className="text-3xl font-bold text-white">Find your next course</h1>
          <form onSubmit={handleSearch} className="flex gap-2 max-w-xl mx-auto">
            ...
            <button type="submit" className="bg-white/20 hover:bg-white/30 text-white px-5 py-3 rounded-xl text-sm font-semibold transition">
              Search
            </button>
          </form>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex gap-3 mb-6 flex-wrap items-center">
          <div className="flex gap-1 flex-wrap">
            {CATEGORIES.map(c => (
              <button key={c} onClick={() => { setCategory(c); setPage(1); }}
                className={`px-3 py-1.5 text-xs font-medium rounded-xl transition ${category === c ? "bg-indigo-600 text-white" : "bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:border-indigo-300"}`}>
                {c}
              </button>
            ))}
          </div>
          <div className="ml-auto flex items-center gap-2">
            <label className="flex items-center gap-2 cursor-pointer text-sm text-gray-600 dark:text-gray-300">
              <input type="checkbox" checked={free} onChange={e => { setFree(e.target.checked); setPage(1); }} className="accent-indigo-600" />
              Free only
            </label>
        ...
        {pagination && pagination.pages > 1 && (
          <div className="flex items-center justify-center gap-2 mt-10">
            ...
            <span className="text-sm text-gray-500 px-2">Page {page} of {pagination.pages}</span>
