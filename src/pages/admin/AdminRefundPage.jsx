...
const STATUS_STYLES = {
  pending:  { pill: "bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300",  dot: "bg-amber-500" },
  approved: { pill: "bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-400",  dot: "bg-green-500" },
  rejected: { pill: "bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-400",           dot: "bg-red-500" },
};
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg p-6 z-10">
        ...
        <div className="flex items-start justify-between mb-5">
          <div>
            <h2 className="text-lg font-bold text-gray-900 dark:text-white">Resolve Refund Request</h2>
            <p className="text-sm text-gray-500 mt-0.5 line-clamp-1">{refund.course?.title}</p>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 p-1 rounded-lg transition">
        ...
        <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 mb-5 grid grid-cols-2 gap-3 text-sm">
          <div>
            ...
            <p className="font-medium text-gray-800 dark:text-white">{refund.student?.name}</p>
            <p className="text-xs text-gray-500">{refund.student?.email}</p>
          </div>
          <div>
            <p className="text-xs text-gray-400 mb-0.5">Amount Paid</p>
            <p className="font-semibold text-gray-800 dark:text-white text-base">₹{refund.amountPaid?.toLocaleString()}</p>
          </div>
          <div>
            <p className="text-xs text-gray-400 mb-0.5">Progress at Request</p>
            <p className="font-medium text-gray-800 dark:text-white">{refund.progressPercentAtRequest}%</p>
          </div>
          <div>
            <p className="text-xs text-gray-400 mb-0.5">Payment Method</p>
            <p className="font-medium text-gray-800 dark:text-white capitalize">{refund.paymentMethod || "—"}</p>
          </div>
          <div className="col-span-2">
            <p className="text-xs text-gray-400 mb-0.5">Student's Reason</p>
            <p className="text-gray-700 dark:text-gray-300 text-xs leading-relaxed">{refund.reason}</p>
          </div>
        </div>
        ...
        {action === "approve" && (
          <div className="mb-4">
            ...
            <input
              type="number"
              value={refundAmount}
              onChange={(e) => setRefundAmount(Math.min(Number(e.target.value), refund.amountPaid))}
              min={0}
            ...
            />
            <p className="text-xs text-gray-400 mt-1">Max: ₹{refund.amountPaid?.toLocaleString()}. Reduce for a partial refund.</p>
          </div>
        )}
        ...
        <div className="mb-5">
          ...
          <textarea
            ...
            rows={3}
            placeholder={action === "approve" ? "e.g. Refund approved. Amount will be credited in 5–7 business days." : "e.g. Refund rejected because progress exceeds the eligible threshold."}
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-2.5 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 rounded-xl text-sm font-semibold hover:bg-gray-50 dark:hover:bg-gray-800 transition">
            Cancel
          </button>
          <button
    ...
    (page = 1) => {
      ...
      Promise.all([
        ...
        .then(([refundData, statsData]) => {
          ...
          setPagination(refundData.pagination);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 py-8 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">Review and resolve student refund requests.</p>
        </div>
        ...
        {stats && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {[
              ...
              { label: "Total Refunded", value: `₹${stats.totalRefunded?.toLocaleString()}`, color: "text-indigo-500" },
            ].map((s) => (
              <div key={s.label} className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl p-4 text-center">
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {refunds.map((refund) => {
              ...
              return (
                <div key={refund._id} className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl p-5 hover:border-indigo-200 dark:hover:border-indigo-800/50 transition">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-3 flex-wrap">
                        <div>
                          <p className="font-semibold text-gray-900 dark:text-white text-sm">{refund.course?.title || "Course"}</p>
                          <div className="flex items-center gap-2 mt-0.5">
                            {refund.student?.avatar?.url ? (
                              <img src={refund.student.avatar.url} className="w-4 h-4 rounded-full" alt="" />
                            ) : (
                            ...
                            <span className="text-xs text-gray-500">{refund.student?.name} · {refund.student?.email}</span>
                          </div>
                        </div>
                      ...
                      <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500">
                        <span>Paid: <span className="font-semibold text-gray-700 dark:text-gray-300">₹{refund.amountPaid?.toLocaleString()}</span></span>
                        <span>Progress: <span className="font-semibold text-gray-700 dark:text-gray-300">{refund.progressPercentAtRequest}%</span></span>
                        <span>Method: <span className="font-semibold text-gray-700 dark:text-gray-300 capitalize">{refund.paymentMethod}</span></span>
                        <span>Requested: <span className="font-semibold text-gray-700 dark:text-gray-300">{new Date(refund.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</span></span>
                      </div>
      ...
      {selected && (
        <ResolveModal
          ...
          onResolved={() => { setSelected(null); load(pagination.page); }}
        />
      )}
    </div>
  );
