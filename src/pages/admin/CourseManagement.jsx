...
export default function CourseManagement() {
  ...
  const load = (overrides = {}) => {
    ...
    const currentFilter    = overrides.filter    !== undefined ? overrides.filter    : publishedFilter;
    const currentPage      = overrides.page      !== undefined ? overrides.page      : page;

    // Only send isPublished when explicitly filtering — omit it for "all"
    const params = {
      limit: 20,
      page: currentPage,
    };
    if (currentSearch.trim()) params.search = currentSearch.trim();
    if (currentFilter === "true")  params.isPublished = true;
    if (currentFilter === "false") params.isPublished = false;

    setIsLoading(true);
    adminFetchCourses(params)
      .then(d => { setCourses(d.courses || []); setPagination(d.pagination); })
      .catch(() => toast.error("Failed to load courses"))
      .finally(() => setIsLoading(false));
  };

  // Re-run whenever search / filter / page changes
