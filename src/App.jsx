...
import PublicRoute from "./routes/PublicRoute";
import ProtectedRoute from "./routes/ProtectedRoute";
import InternRoute from "./routes/InternRoutes";
import Navbar from "./components/Navbar";
import Footer from "./components/Footer";
import StudentLayout from "./components/layout/StudentLayout";
import CreatorLayout from "./components/layout/CreatorLayout";
...
// Live Classroom (shared by intern-portal students AND admin hosts — the
// server-side /join endpoint enforces who actually gets a token, not routing)
const LiveClassroomPage       = lazy(() => import(/* webpackPrefetch: true */ "./pages/Intern/LiveClassRoomPage"));

// Intern Portal
const InternDashboard         = lazy(() => import(/* webpackPrefetch: true */ "./pages/Intern/InternDashboard"));
const InternLiveClasses       = lazy(() => import(/* webpackPrefetch: true */ "./components/ui/InternLiveClasses"));
...
const AdminWallets          = lazy(() => import(/* webpackPrefetch: true */ "./pages/admin/AdminWallets"));
const InternshipManagement  = lazy(() => import(/* webpackPrefetch: true */ "./pages/admin/InternshipManagement"));
const LiveClassManagement   = lazy(() => import(/* webpackPrefetch: true */ "./pages/admin/LiveClassRoomManagement"));
const HeroImageManager      = lazy(() => import(/* webpackPrefetch: true */ "./pages/admin/HeroImageManager"));
const AdminRefundPage       = lazy(() => import(/* webpackPrefetch: true */ "./pages/admin/AdminRefundPage"));
const AdminIssueCertificate = lazy(() => import(/* webpackPrefetch: true */ "./pages/admin/AdminIssueCertificate"));
const AdminProfilePage      = lazy(() => import(/* webpackPrefetch: true */ "./pages/admin/AdminProfilePage"));
  ...
  return (
    <>
      ...
      <Routes>
        ...
        {/* ── Live Classroom — full-screen, no Navbar/Footer/sidebar.
             Open to any authenticated role; real host-vs-participant
             authorization happens server-side in POST /live-classes/:id/join,
             not here. ─────────────────────────────────────────────────── */}
        <Route element={<ProtectedRoute allowedRoles={["student", "creator", "admin"]} />}>
          <Route path="/classroom/:id" element={<LiveClassroomPage />} />
        </Route>

        {/* ── Intern Portal — gated by InternRoute (shortlisted + paid-if-
             required + linked account), independent of course role. ────── */}
        <Route element={<InternRoute />}>
          <Route element={<InternLayout />}>
            <Route path="/intern/dashboard"       element={<InternDashboard />} />
            <Route path="/intern/live-classes"    element={<InternLiveClasses />} />
          </Route>
        </Route>
