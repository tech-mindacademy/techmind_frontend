...
import { getAssignment, submitAssignment } from "../../api/services/assignment.service";
...
export default function AssignmentPanel({ assignmentId, courseId }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await submitAssignment(assignmentId, fd, (pct) => setUploadProgress(pct));
  ...
  if (!assignment) return (
    <p className="text-sm text-gray-500 text-center py-8">Assignment not found.</p>
  );

  const isOverdue = assignment.dueDate && !assignment.allowLateSubmission && new Date() > new Date(assignment.dueDate);
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="flex gap-3 mt-1.5 text-xs text-gray-500">
          <span>Max marks: <span className="text-gray-300">{assignment.maxMarks}</span></span>
        </div>
      </div>
      ...
      {assignment.attachments?.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Reference Files</p>
          <div className="space-y-1.5">
            {assignment.attachments.map(att => (
              <a key={att._id} href={att.url} target="_blank" rel="noreferrer"
                ...
                <span className="text-sm text-gray-200 truncate">{att.title}</span>
                <span className="text-xs text-gray-500 uppercase ml-auto">.{att.fileType}</span>
              </a>
            ))}
          </div>
        </div>
      ...
      {mySubmission && (
        <div className={`rounded-xl p-3 ${statusColors[mySubmission.status]}`}>
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold">{statusLabels[mySubmission.status]}</p>
            {mySubmission.grade !== null && (
              <span className="text-sm font-bold">{mySubmission.grade} / {assignment.maxMarks}</span>
            )}
          </div>
          ...
          {mySubmission.submittedAt && (
            <p className="text-xs mt-1 opacity-60">Submitted {new Date(mySubmission.submittedAt).toLocaleDateString()}</p>
          )}
        </div>
      )}
          ...
          <div>
            ...
            {files.length > 0 && (
              <div className="mt-2 space-y-1">
                {files.map((f, i) => (
                  <div key={i} className="flex items-center justify-between text-xs text-gray-400 bg-gray-800 rounded-lg px-2 py-1.5">
                    <span className="truncate">{f.name}</span>
                    <button onClick={() => setFiles(prev => prev.filter((_, j) => j !== i))} className="text-gray-500 hover:text-red-400 ml-2 flex-shrink-0">✕</button>
                  </div>
                ))}
              </div>
            )}
