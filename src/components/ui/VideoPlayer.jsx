...
export default function VideoPlayer({ src, onEnded, className = "" }) {
  ...
  const handlePause = () => { setIsPlaying(false); setShowControls(true); clearTimeout(hideControlsRef.current); };
  ...
  const handleError = () => {
    if (!isOnline) return; // handled by offline UI
    ...
    >
      ...
      {error && isOnline && (
        <div className="absolute inset-0 bg-gray-950/90 flex flex-col items-center justify-center gap-3 z-20">
          ...
          <button onClick={(e) => { e.stopPropagation(); setError(null); videoRef.current?.load(); }}
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 px-4 pb-3 space-y-2">
          ...
          <div ref={progressRef} onClick={seek}
            ...
            <div className="absolute top-1/2 -translate-y-1/2 w-3 h-3 bg-white rounded-full shadow opacity-0 group-hover:opacity-100 transition"
          ...
          <div className="flex items-center gap-3">
            {/* Play/Pause */}
            <button onClick={togglePlay} className="text-white hover:text-indigo-300 transition flex-shrink-0">
              {isPlaying ? (
            ...
            {/* Skip back */}
            <button onClick={() => skip(-10)} className="text-white/70 hover:text-white transition flex-shrink-0">
            ...
            {/* Volume */}
            <button onClick={toggleMute} className="text-white/70 hover:text-white transition flex-shrink-0">
              {isMuted || volume === 0 ? (
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
